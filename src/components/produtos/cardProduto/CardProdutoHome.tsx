...
function CardProdutoHome({ post }: CardProdutoProps) {
  ...
  return (
    <div className="flex flex-col border rounded-xl overflow-hidden justify-between p-2 w-[14rem] h-auto">
      <div>
        <div className="flex w-full h-[10rem] justify-center ">
          <img
        ...
        <div className="my-2">
          <h4 className="text-lg h-[3.5rem] line-clamp-2 mb-2 text-slate-800">
            {post.name} - {post.user?.name}
          </h4>
          <p className="ml-[4px] text-cor-primaria font-bold text-2xl">
      ...
      {tarefa ? (
        <div>
          <button
            className="border-2 border-lime-950 rounded-lg w-full h-[3rem] font-bold text-lg from-green-950 hover:bg-lime-950 hover:text-white hover:transition-all "
            onClick={() => setValor(1)}
          >
            Adicionar
          </button>
        ...
        <div className="flex justify-between border-2 border-lime-950 rounded-lg w-full h-[3rem] font-bold text-lg from-green-950 items-center p-2 text-center">
          ...
          <button className="text-2xl" onClick={somarQuantidade}>
            +
          </button>
        </div>
      )}
